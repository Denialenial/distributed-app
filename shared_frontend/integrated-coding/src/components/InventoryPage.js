...
const InventoryPage = () => {
  ...
  const fetchInventories = async () => {
    try {
      const response = await axios.get('http://localhost:5003/api/inventories');
  ...
  const handleAdd = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:5003/api/inventories', newInventory);
  ...
  const handleRemove = async (id) => {
    try {
      const response = await axios.delete(`http://localhost:5003/api/inventories/${id}`, {
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`http://localhost:5003/api/inventories/${editingInventory._id}`, editingInventory);
